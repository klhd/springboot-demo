...
function assertProp (
  ...
  if (validator) {
    if (!validator(value)) {
      warn(
        'Invalid prop: psi validator check failed for prop "' + name + '".',
        vm
      );
    }
  }
    ...
    if (isDef(tag)) {
      {
        ...
        if (
          ...
          warn(
            'Unknown psi element: <' + tag + '> - did you ' +
...
function model (
  ...
  if (tag === 'select') {
    ...
    warn$1(
      ...
      'wrap a library dedicated for that purpose inside a psi component.'
    );
  }
